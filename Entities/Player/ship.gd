...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_pressed("accelerate"):
		...
		throttle_velocity -= idle_deccel * throttle_velocity.normalized() * delta
	
	apply_speed_booster_boost(delta)
	ship_velocity -= ship_deccel * delta * ship_velocity.normalized()
...
func apply_speed_booster_boost(delta: float) -> void:
	for area in collision_detector.get_overlapping_areas():
		if area.is_in_group("speed_booster"):
			ship_velocity += area.speed_boost * delta * Vector2.from_angle(area.rotation-PI/2)
